...
// ─────────────────────────────────────────────────────────────
// 6. TECHNICAL SPECIFICATIONS (per package — highlighted)
// ─────────────────────────────────────────────────────────────
function TechnicalSpecificationsByPackage({ lang }: { lang: Lang }) {
  const t = useT(lang);

  const packages = [
    {
      name: "Calathea",
      tier: { id: "Paket Entry", en: "Entry Package" },
      equipment: [
        { id: "Mesin komersial LG", en: "LG commercial machines" },
        { id: "Mesin Speed Queen", en: "Speed Queen machines" },
      ],
      configuration: {
        id: "Setup laundry profesional (sistem mesin cuci + pengering)",
        en: "Professional laundry setup (washer + dryer system)",
      },
      highlights: [
        {
          id: "Fokus efisiensi operasional",
          en: "Operational efficiency focus",
        },
        { id: "Daya tahan & keandalan", en: "Durability & reliability" },
      ],
    },
    {
      name: "Caladium",
      tier: { id: "Paket Standard", en: "Standard Package" },
      equipment: [
        { id: "Mesin komersial LG", en: "LG commercial machines" },
        { id: "Sistem Maytag Full Stack", en: "Maytag full stack system" },
      ],
      configuration: {
        id: "5 stack mesin cuci & pengering",
        en: "5-stack washer & dryer configuration",
      },
      highlights: [
        {
          id: "Throughput kapasitas tinggi",
          en: "High-capacity throughput",
        },
        {
          id: "Dioptimalkan untuk operasi volume",
          en: "Optimized for volume operations",
        },
        {
    ...
    {
      name: "Monstera",
      tier: { id: "Paket Premium", en: "Premium Package" },
      equipment: [
        { id: "Mesin komersial Huebsch", en: "Huebsch commercial machines" },
      ],
      configuration: null,
      highlights: [
        {
          id: "Mesin industri kelas premium",
          en: "Premium industrial-grade machines",
        },
        { id: "Daya tahan tinggi", en: "High durability" },
        {
          id: "Dirancang untuk operasi skala besar",
  ...
  const systemSpecs = [
    {
      ...
      title: { id: "Aplikasi Digital", en: "Digital Application" },
      desc: {
        id: "Pemesanan, pelacakan, dan pembayaran real-time.",
        en: "Real-time ordering, tracking, and payments.",
      },
    },
    {
      icon: (
      ...
      title: { id: "POS Terintegrasi", en: "Integrated POS" },
      desc: {
        id: "Kasir, inventori, dan laporan cabang.",
        en: "Checkout, inventory, and branch reporting.",
      },
    },
    {
      icon: (
      ...
      desc: {
        id: "SOP & checklist di setiap tahap.",
        en: "SOPs & checklists at every stage.",
      },
    },
    {
      icon: (
      ...
      desc: {
        id: "Operasional, layanan, keuangan, branding.",
        en: "Operations, service, finance, branding.",
      },
    },
  ];
  ...
  return (
    <section
      ...
      className="py-20 md:py-32 bg-[#0D1B2A] relative overflow-hidden border-y border-[#F5A623]/25"
    >
      <div
      ...
      <div className="relative container">
        <motion.div {...fadeUp()} className="max-w-3xl mb-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 border border-[#F5A623]/40 text-[#F5A623] text-[10px] font-bold tracking-[0.22em] uppercase mb-6">
            {t({ id: "Fokus Investasi", en: "Investment Focus" })}
          </div>
          <SectionLabel>
            <span className="text-[#F5A623]">
          ...
          >
            {t({
              id: "Perangkat & Konfigurasi per Paket Kemitraan",
              en: "Equipment & Configuration by Partnership Package",
            })}
          </h2>
          <p className="text-white/50 leading-relaxed text-base max-w-2xl">
            {t({
              id: "Setiap paket menggunakan merek dan tata letak mesin yang disesuaikan dengan skala investasi, throughput harian, dan target pasar mitra.",
              en: "Each package uses brands and machine layouts matched to investment scale, daily throughput, and the partner's target market.",
            })}
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 mb-16 lg:mb-20">
          {packages.map((pkg, i) => (
            <motion.article
              key={pkg.name}
              {...fadeUp(0.08 + i * 0.06)}
              className="flex flex-col border border-white/12 bg-[#0D1B2A] hover:border-[#F5A623]/35 transition-colors duration-300"
            >
              <header className="px-6 pt-8 pb-6 border-b border-white/10">
                <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-[#F5A623]/80 mb-2">
                  {t(pkg.tier)}
                </p>
                <h3
              ...
              <div className="px-6 py-6 flex-1 flex flex-col gap-6">
                <div>
                  <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-white/35 mb-3">
                    {t({ id: "Peralatan", en: "Equipment" })}
                  </p>
                  <ul className="space-y-2">
                    {pkg.equipment.map((line) => (
                      <li
                        key={t(line)}
                        className="text-sm text-white/80 flex gap-2 items-start"
                      >
                        <span className="text-[#F5A623] mt-1.5 w-1 h-1 rounded-full bg-[#F5A623] flex-shrink-0" />
                        {t(line)}
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-white/35 mb-3">
                    {t({ id: "Poin Utama", en: "Highlights" })}
                  </p>
                  <ul className="space-y-2">
                    {pkg.highlights.map((h) => (
                      <li
                        key={t(h)}
                        className="text-sm text-white/60 leading-relaxed pl-3 border-l border-[#F5A623]/40"
                      >
                        {t(h)}
                      </li>
                    ))}
        ...
        <motion.div {...fadeUp(0.2)}>
          <h3
            className="font-bold text-white mb-6 text-center lg:text-left"
            style={{
              fontFamily: "var(--font-playfair), serif",
              fontSize: "1.25rem",
            }}
          >
            {t({
              id: "Sistem, Proses & Teknologi Pendukung",
              en: "Supporting Systems, Process & Technology",
            })}
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-white/10">
            {systemSpecs.map(({ icon, title, desc }, j) => (
              <div
                key={t(title)}
                className="bg-[#0D1B2A] p-6 flex gap-4 items-start"
              >
                <div className="text-[#F5A623] flex-shrink-0 mt-0.5">{icon}</div>
                <div>
                  <p
...
// ─────────────────────────────────────────────────────────────
// 7. PERLENGKAPAN PRODUKSI (Production equipment)
// ─────────────────────────────────────────────────────────────
function ProductionEquipment({ lang }: { lang: Lang }) {
  const t = useT(lang);

  const items: { title: Bilingual; note?: Bilingual }[] = [
    {
      title: {
        id: "Mesin cuci & pengering (industrial)",
        en: "Industrial washers & dryers",
      },
    },
    {
      title: {
        id: "Meja sortir & lipat",
        en: "Sorting & folding tables",
      },
    },
    {
      title: {
    ...
    },
    {
      title: {
        id: "Sistem kasir / POS",
        en: "Cash register / POS system",
      },
    },
    {
      title: {
        id: "Peralatan operasional harian",
        en: "Daily operational tools",
      },
    },
  ];
  ...
  return (
    <section
      id="perlengkapan-produksi"
      className="py-20 md:py-32 bg-[#FAF8F4] border-b border-slate-200/80"
    >
      <div className="container">
        <motion.div {...fadeUp()} className="max-w-2xl mb-12 md:mb-16">
          ...
          >
            {t({
              id: "Peralatan Operasional Lengkap",
              en: "Complete Operational Toolkit",
            })}
          </h2>
          <p className="text-slate-500 leading-relaxed">
            {t({
              id: "Daftar perlengkapan standar yang mendukung alur produksi harian — dari penerimaan hingga pengemasan.",
              en: "Standard equipment supporting the daily production flow — from intake through to packaging.",
            })}
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-slate-200">
          {items.map(({ title, note }, i) => (
            <motion.div
              key={t(title)}
              {...fadeUp(i * 0.05)}
              className="bg-white p-8 flex gap-5 items-start hover:bg-[#FAF8F4] transition-colors duration-200"
            >
              <div
                className="flex-shrink-0 w-9 h-9 border border-slate-200 flex items-center justify-center"
                aria-hidden="true"
              >
                <span className="block w-2 h-2 bg-[#F5A623]" />
              </div>
              <div>
                <h3
                  className="font-semibold text-[#0D1B2A] text-[0.95rem] leading-snug mb-1"
                  style={{ fontFamily: "var(--font-playfair), serif" }}
                >
                  {t(title)}
                </h3>
...
export default function KemitraanClient() {
  ...
  return (
    <>
      ...
      <TechnicalSpecificationsByPackage lang={lang} />
